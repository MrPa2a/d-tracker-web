...
import { Zap, Play, Trophy, Clock } from 'lucide-react';

interface SpeedMarketPageProps {
  server: string | null;
  currentProfile: Profile | null;
}

export const SpeedMarketPage = ({ server, currentProfile }: SpeedMarketPageProps) => {
  return (
      ...
      <div className="bg-bg-secondary border border-border-normal rounded-xl p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <h2 className="text-xl font-bold text-text-primary">Speed Market</h2>
            <p className="text-text-muted mt-1">
              Classe les items du moins cher au plus cher le plus rapidement possible !
            </p>
          </div>
        </div>
      ...
      <div className="bg-bg-secondary border border-border-normal rounded-xl p-8 min-h-[400px] flex flex-col items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-text-muted text-sm mt-1">Le jeu sera implémenté dans le Module 4</p>
          </div>
          <button className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-orange-500 to-amber-500 text-white font-medium rounded-lg hover:opacity-90 transition-opacity">
            <Play size={20} />
