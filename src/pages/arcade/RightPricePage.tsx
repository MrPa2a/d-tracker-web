...
import { TrendingUp, Play, Trophy, Clock } from 'lucide-react';

interface RightPricePageProps {
  server: string | null;
  currentProfile: Profile | null;
}

export const RightPricePage = ({ server, currentProfile }: RightPricePageProps) => {
  return (
      ...
      <div className="bg-bg-secondary border border-border-normal rounded-xl p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <h2 className="text-xl font-bold text-text-primary">Le Juste Prix</h2>
            <p className="text-text-muted mt-1">
              Estime le prix actuel d'un item sur le marché. Moins d'essais = plus de points !
            </p>
          </div>
        </div>
      ...
      <div className="bg-bg-secondary border border-border-normal rounded-xl p-8 min-h-[400px] flex flex-col items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-text-muted text-sm mt-1">Le jeu sera implémenté dans le Module 3</p>
          </div>
          <button className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-medium rounded-lg hover:opacity-90 transition-opacity">
            <Play size={20} />
