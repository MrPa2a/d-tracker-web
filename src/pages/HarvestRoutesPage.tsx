...
export const HarvestRoutesPage: React.FC = () => {
  ...
  const [maxMoves, setMaxMoves] = useState<number>(20);
  const [startX, setStartX] = useState<number>(0);
  const [startY, setStartY] = useState<number>(0);

  // Données et mutations
  const { data: jobs = [], isLoading: isLoadingJobs } = useHarvestJobs();
  const { data: resources = [], isLoading: isLoadingResources } = useHarvestResources(
    selectedJobIds,
    levelMin,
    levelMax
  );
  ...
  // Grouper les ressources par métier
          ...
          <div className="mt-4 border-t border-white/5 pt-4">
            ...
            {isLoadingResources ? (
              ...
              <div className="space-y-4">
                {Array.from(resourcesByJob.entries()).map(([jobId, jobResources]) => {
